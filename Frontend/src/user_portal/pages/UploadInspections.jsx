...
const UploadInspections = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const type = new URLSearchParams(window.location.search).get("type");
      const id = new URLSearchParams(window.location.search).get("id");
      const reminder_id = new URLSearchParams(window.location.search).get(
        "r_id"
      );
      ...
      const uploadPromises = files.map(async (file) => {
        const storageRef = ref(
          storage,
          `${getType(type)}/${timestamp}_${uniqueId}_${file.name}`
        );
            ...
            <div className="w-full flex flex-col justify-center p-4 items-start shadow-lg rounded-lg">
              <Typography variant="h6">
                <span className="font-semibold">
                  Note: Your Requested Quote for type:{" "}
                </span>
                {documentData.policyType}{" "}
                <span className="font-semibold">with Id: </span>{" "}
                {documentData.id} cannot be processed further until you have
                submit required Inspections with it.
              </Typography>
              ...
              {documentData.persons &&
                documentData.persons.map((person, index) => (
              ...
              {documentData.drivers &&
                documentData.drivers.map((driver, index) => (
                  <React.Fragment key={`driver-${index}`}>
                    ...
                    <div className="w-full grid grid-cols-1 mt-[10px] mb-[20px] lg:grid-cols-2 gap-5 justify-center items-center">
                      ...
                      {driver.phoneNumber &&
                        renderTextField(
                          `Driver Phone Number ${index + 1}`,
                          driver.phoneNumber
                        )}
                      {driver.LN &&
                        renderTextField(
                          `Driver License Number ${index + 1}`,
                          driver.LN
                        )}
                    </div>
                  </React.Fragment>
              ...
              {documentData.address &&
                renderTextField("Address to be insured", documentData.address)}
                    ...
                    <Grid container spacing={2} justifyContent="center">
                      {files.map((file, index) => (
                        <Grid item xs={6} sm={4} md={3} key={index}>
                          <div className="flex flex-col items-center justify-center border rounded-lg p-2 shadow-md bg-gray-100">
                            {/* Tooltip for Long File Names */}
                            <Tooltip title={file.name} arrow>
                              <p className="text-sm font-medium text-center w-full truncate max-w-[100px]">
                                {file.name}
                              </p>
                            </Tooltip>
